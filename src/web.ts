...
export class CapacitorNFCPassWalletWeb
  extends WebPlugin
  implements CapacitorNFCPassWalletPlugin
{
  async savePassToWallet(_options: SavePassToWalletOptions): Promise<void> {
    throw this.toUnavailableError(
      "Wallet operations require native iOS/Android implementation.",
    );
  }

  async readPassFromWallet(
    _options: ReadPassFromWalletOptions,
